#!/usr/bin/env python3
# scrapers/reebee_scraper.py — Reebee flyer scraper (Canada)
# Covers: FreshCo, Sobeys, Food Basics, Giant Tiger, Farm Boy, IGA, Safeway CA
...
# Reebee-network stores available via Flipp
REEBEE_STORES = {
    "sobeys", "freshco", "fresh co", "food basics", "giant tiger",
    "farm boy", "iga", "safeway", "thrifty foods", "foody world",
    "price chopper", "co-op", "overwaitea",
}


def search_reebee(query: str, postal_code: str, keyword_map: dict = None) -> List[Dict]:
    """
    Pull Reebee-network stores (Sobeys, FreshCo, Food Basics, Giant Tiger etc.)
    via Flipp, which aggregates all these chains.
    """
    results = []
    seen = set()
    ...
    for term in terms[:3]:
        try:
            ...
            for item in items:
                ...
                if not any(s in store for s in REEBEE_STORES):
                    continue
                ...
                url      = f"https://flipp.com/en-ca/flyer/{flyer_id}" if flyer_id else "https://reebee.com"
                ...
                results.append({
                    "name":        name,
                    "store":       item.get("merchant_name") or store.title(),
            ...
            print(f"[Reebee] Error for '{term}': {e}")
